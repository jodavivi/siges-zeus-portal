 sap.ui.define([ 
	...
	"use strict";
	return {
		consultarRoles: function(oParam, callback,context) {
			UtilHttp.httpGet(Constantes.services.consultarRol+UtilUi.geneararFiltro(oParam) , Constantes.IdApp, function(result) {
		...
		consultarRolAplicacion: function(oParam, callback,context) {
			UtilHttp.httpGet(Constantes.services.consultarRolAplicacion+UtilUi.geneararFiltro(oParam) , Constantes.IdApp, function(result) {
				var oAuditResponse = result.oAuditResponse;
				if (oAuditResponse.iCode === 1) {
					callback(UtilResponse.success(oAuditResponse.sIdTransaction, oAuditResponse.sMessage, result.oData));
				} else if (oAuditResponse.iCode > 1) {

 sap.ui.define([ 
	...
	"use strict";
	return {
		consultarCategoria: function(oParam, callback,context) {
			UtilHttp.httpGet(Constantes.services.consultarCategoria+UtilUi.geneararFiltro(oParam) , Constantes.IdApp, function(result) {
		...
		registrarCategoria: function(oParam, callback,context) {
			UtilHttp.httpPost(Constantes.services.registrarCategoria, oParam, Constantes.IdApp, function(result) {
				var oAuditResponse = result.oAuditResponse;
				if (oAuditResponse.iCode === 1) {
					callback(UtilResponse.success(oAuditResponse.sIdTransaction, oAuditResponse.sMessage, result.oData));
				} else if (oAuditResponse.iCode > 1) {
		...
		actualizarCategoria: function(oParam, callback,context) {
			UtilHttp.httpPut(Constantes.services.actualizarCategoria.replace("{0}", oParam.iId), oParam, Constantes.IdApp, function(result) {
				var oAuditResponse = result.oAuditResponse;
				if (oAuditResponse.iCode === 1) {
					callback(UtilResponse.success(oAuditResponse.sIdTransaction, oAuditResponse.sMessage, result.oData));
				} else if (oAuditResponse.iCode > 1) {
		...
		eliminarCategoria: function(oParam, callback,context) {
			UtilHttp.httpDelete(Constantes.services.eliminarCategoria, oParam, Constantes.IdApp, function(result) {
				var oAuditResponse = result.oAuditResponse;
				if (oAuditResponse.iCode === 1) {
					callback(UtilResponse.success(oAuditResponse.sIdTransaction, oAuditResponse.sMessage, result.oData));
				} else if (oAuditResponse.iCode > 1) {
